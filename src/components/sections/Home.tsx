  ...
  return (
    ...
    >
      <div className="flex-1 w-full text-left max-w-2xl md:max-w-xl flex flex-col justify-between min-h-0 self-stretch">
        <p className="text-sm uppercase tracking-[0.2em] sm:tracking-[0.25em] text-[var(--accent)] mt-8 md:mt-0 mb-4 md:mb-6 font-medium">
          Portfolio
        </p>
        <p className="text-xl md:text-lg text-[var(--muted)] mb-3 md:mb-3">
          {t('home.greeting')}
        </p>
        ...
        <p className="text-lg md:text-lg text-[var(--muted)] max-w-lg mb-6 md:mb-10 leading-relaxed md:leading-loose">
          {t('home.description')}
        </p>
        <div className="flex flex-col sm:flex-row flex-wrap gap-4 sm:gap-5 mb-8 md:mb-10">
          ...
          <a
            href="#contact"
            className="inline-flex items-center justify-center w-full sm:w-auto px-6 py-3 sm:px-10 sm:py-5 text-base sm:text-lg font-medium border-2 border-[var(--fg)]/30 rounded-full hover:border-[var(--fg)]/50 transition-colors"
          >
            {t('home.cta_contact')}
          </a>
        </div>
        <div className="flex gap-10 text-white">
          <a
            href="https://www.linkedin.com/in/h%C3%A9di-boissard-a994ab25b/"
            target="_blank"
            rel="noopener noreferrer"
            className="text-white hover:opacity-80 transition-opacity p-3 -m-3"
            aria-label="LinkedIn"
          >
            <img src={`${import.meta.env.BASE_URL}assets/Linkedin.svg`} alt="" className="w-8 h-8 md:w-7 md:h-7 brightness-0 invert" />
          </a>
          <a
            ...
            aria-label="GitHub"
          >
            <img src={`${import.meta.env.BASE_URL}assets/GitHub.svg`} alt="" className="w-8 h-8 md:w-7 md:h-7 brightness-0 invert" />
          </a>
        </div>
      </div>
