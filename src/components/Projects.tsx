...
export default function Projects() {
  ...
  const handleTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    // Swipe horizontal significatif uniquement
  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto flex flex-col items-center md:items-stretch">
        <h2 className="w-full text-left text-2xl md:text-3xl font-semibold tracking-tight text-[var(--fg)] mb-10">
          <BlurText
        ...
        >
          ...
          <AnimatePresence mode="wait" initial={false} custom={direction}>
            <motion.article
              ...
              className="relative w-full max-w-sm sm:max-w-md md:max-w-3xl rounded-3xl bg-[#101320] border border-white/5 shadow-[0_20px_45px_rgba(15,23,42,0.9)] px-4 sm:px-6 md:px-10 py-6 md:py-8"
            >
              <h3 className="text-xl md:text-2xl font-semibold text-[var(--fg)] mb-2">
                {currentProject.title}
              </h3>
              <p className="text-sm md:text-base text-[var(--muted)] mb-4 md:mb-5">
              ...
              <div className="flex flex-wrap gap-2 mb-5">
                {currentProject.tech.map((tech) => (
                  <span
                    key={tech}
                    className="text-xs md:text-[13px] px-3 py-1.5 rounded-full bg-[var(--accent)]/15 text-[var(--fg)] font-medium"
                  >
                    {tech}
                  </span>
                ))}
              ...
              <a
                ...
                className="group mt-2 block w-full rounded-full bg-black/60 border border-black/60 hover:border-[var(--accent)]/60 px-5 py-3 text-sm md:text-[15px] text-[var(--fg)] transition-colors"
              >
